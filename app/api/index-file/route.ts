...
export async function POST(request: NextRequest) {
  try {
    ...
    let errorMessage = "Failed to index file";
    
    if (error instanceof Error) {
      if (error.message.includes("OpenAI")) {
        errorMessage = "Invalid OpenAI API key or OpenAI service error";
      } else if (error.message.includes("Qdrant") || error.message.includes("vector")) {
        errorMessage = "Vector database connection error. Please check if Qdrant is running.";
      } else if (error.message.includes("PDF") || error.message.includes("parse")) {
        errorMessage = "File parsing error. Please ensure the file is not corrupted.";
      } else if (error.message.includes("ENOENT") || error.message.includes("file")) {
        errorMessage = "File system error. Please try again.";
      } else {
        errorMessage = `File indexing failed: ${error.message}`;
      }
    }
